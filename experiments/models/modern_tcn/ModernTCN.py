...
import math
from experiments.models.modern_tcn.RevIN import RevIN
from experiments.models.modern_tcn.ModernTCN_Layer import series_decomp, Flatten_Head
...
class ModernTCN(nn.Module):
    def __init__(self,task_name,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
        ...
        if self.task_name == 'regression':
            self.act_reg = F.gelu
            self.reg_dropout = nn.Dropout(self.class_drop)
            _output_size = output_size if output_size is not None else 1
            self.head_reg = nn.Linear(self.n_vars[0] * self.head_nf, _output_size)
