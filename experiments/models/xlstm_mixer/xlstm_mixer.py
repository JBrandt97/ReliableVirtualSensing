...
from experiments.models.xlstm_mixer.Autoformer_EncDec import series_decomp
from experiments.models.xlstm_mixer.StandardNorm import Normalize as RevIN
...
from experiments.models.xlstm_mixer.base_model import BaseModel
...
class xLSTMMixer(BaseModel):
    ...
    def __init__(
        ...
        # ---- added: single FC regression head for TSER ----
        # Mirrors the paper's classification adaptation: "replace the final
        # regression projection layer with a single fully connected
        # classification head" — here without softmax for regression.
        _head_in = enc_in * (xlstm_embedding_dim * 2 if backcast else xlstm_embedding_dim)
        self.regression_head = nn.Linear(_head_in, 1)
        # ---- end added ----

    # ---- added: encoder path + regression head for TSER ----
    # Mirrors the encoder stages of forecast() but skips RevIN and the
    # forecast projection (self.fc), then applies the single FC regression
    # head. Returns (B, 1).
    def classification(self, x_enc):
        x = self.seq_var_2_var_seq(x_enc)                           # (B,T,C) → (B,C,T)
        x = self.pre_encoding(x)                                    # (B, C, emb)
        ...
        if self.backcast and not self.ablation_mode.no_backcast:
            x_bwd = self.xlstm(torch.flip(x, [dim]))
        ...
        # Single FC regression head 
        x = x.reshape(x.shape[0], -1)
        return self.regression_head(x)                              # (B, 1)
    # ---- end added section ----
